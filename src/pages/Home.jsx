...
export default function Home() {
  ...
  const cmpRows = [
    { key: 'r1', sdm: true,  nula: 'partial', desktop: true,       intl: false },
    { key: 'r2', sdm: true,  nula: 'limited', desktop: 'seplicense', intl: 'addon' },
    { key: 'r3', sdm: true,  nula: 'partial', desktop: true,       intl: 'varies' },
    { key: 'r4', sdm: true,  nula: 'peruser', desktop: 'peruser',  intl: 'peruser' },
    { key: 'r5', sdm: true,  nula: 'limited', desktop: 'partial',  intl: true },
    { key: 'r6', sdm: true,  nula: true,      desktop: false,      intl: true },
    { key: 'r7', sdm: true,  nula: true,      desktop: 'local',    intl: 'varies' },
    { key: 'r8', sdm: true,  nula: true,      desktop: true,       intl: false },
  ]

  function CmpCell({ val }) {
    if (val === true)  return <span className="check-green">✓</span>
    if (val === false) return <span className="cross-red">✗</span>
    return <span className="text-muted">{t(`cmp.${val}`)}</span>
  }
      ...
      <section className="py-20 px-4" style={{ background: '#F5F8FF' }}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-sdm-navy mb-3">{t('cmp.title')}</h2>
            <p className="text-gray-400 text-sm">{t('cmp.note')}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
            <div className="overflow-x-auto">
              <table className="cmp w-full">
                <thead>
                  <tr>
                    <th className="text-left">{t('cmp.feature')}</th>
                    <th className="text-center text-blue-200">SDM</th>
                    <th className="text-center text-gray-300">{t('cmp.col.nula')}</th>
                    <th className="text-center text-gray-300">{t('cmp.col.desktop')}</th>
                    <th className="text-center text-gray-300">{t('cmp.col.intl')}</th>
                  </tr>
                </thead>
                <tbody>
                  {cmpRows.map(({ key, sdm, nula, desktop, intl }) => (
                    <tr key={key}>
                      <td className="font-medium text-gray-700">{t(`cmp.${key}`)}</td>
                      <td className="text-center"><CmpCell val={sdm} /></td>
                      <td className="text-center"><CmpCell val={nula} /></td>
                      <td className="text-center"><CmpCell val={desktop} /></td>
                      <td className="text-center"><CmpCell val={intl} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
